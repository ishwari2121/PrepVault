...
const AptitudeQuestions = () => {
    ...
    useEffect(() => {
        ...
        const savedState = localStorage.getItem("aptitudeQuesState");
        ...
        if (savedState) {
            try {
                const { selectedType: savedType, currentPage: savedPage } = JSON.parse(savedState);
    ...
    useEffect(() => {
        localStorage.setItem(
            "aptitudeQuesState",
            JSON.stringify({ selectedType, currentPage })
        );
    ...
    useEffect(() => {
        if (selectedType) {
            let filtered = allMcq.filter(mcq => mcq.type === selectedType);
            ...
            if (searchTerm) {
                ...
                filtered = filtered.filter(mcq => {
                    ...
                    if (mcq.default_explanation.toLowerCase().includes(searchTermLower)) return true;
    ...
    const handleShareClick = (mcqId) => {
        ...
        if(!user) {
            navigate("/signin/commonQuestion?category=aptitude");
        }
    ...
    const handleSubmitExplanation = async (mcqId) => {
        ...
        try {
            await axios.post(
                `http://localhost:5000/api/aptitude/${mcqId}/explanations`,
    ...
    const handleDeleteExplanation = async (questionId, explanationId) => {
        try {
            await axios.delete(
                `http://localhost:5000/api/aptitude/${questionId}/explanations/${explanationId}`
            );
            fetchAllMcq();
        } catch (error) {
    ...
    return (
        <div className="p-6 bg-gray-900 min-h-screen">
            {/* Heading */}
            <h1 className="text-3xl font-bold text-blue-400 mb-8">Aptitude Questions</h1>
                                    ...
                                    {answeredQuestions[mcq._id] && (
                                        ...
                                        >
                                            ...
                                            <p className="text-gray-300 whitespace-pre-line">
                                                {searchTerm ? highlightSearchTerm(mcq.default_explanation) : mcq.default_explanation}
                                            </p>

                                            {mcq.users_explanation?.length > 0 && (
                                                <div className="mt-6">
                                                    <button
                                                    ...
                                                    >
                                                        ...
                                                        {showUserExplanations[mcq._id] 
                                                            ...
                                                            : `View Community Insights (${mcq.users_explanation.length})`}
                                                    </button>
                                                    ...
                                                    {showUserExplanations[mcq._id] && (
                                                        ...
                                                        >
                                                            ...
                                                            {mcq.users_explanation?.slice().reverse().map((exp, idx) => (
